"use client";

import React, { useState, useCallback, memo } from "react";
import Link from "next/link";
import HeaderSearchBar from "@/src/app/(components)/(header)/HeaderSearchBar";
import { ThemeToggle } from "@/src/app/(components)/(header)/(button)/ThemeToggle";
import styles from "@/src/styles/components/header/Header.module.css";

export interface ComponentHeaderProps {
  device?: "desktop";
  listStatus?: "close";
  className?: string;
}

const Header: React.FC<ComponentHeaderProps> = ({ className = "" }) => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const toggleMenu = useCallback(() => {
    setIsMenuOpen((prev) => !prev);
  }, []);

  const closeMenu = useCallback(() => {
    setIsMenuOpen(false);
  }, []);

  return (
    <header
      className={`${styles.header} ${isMenuOpen ? styles.isMenuOpen : ""} component-header ${className}`}
    >
      <div className={styles.headerMainArea}>
        {/* 1. Logo */}
        <Link
          href="/"
          className={`${styles.iconButton} component-cell-header-search`}
          aria-label="Logo"
          onClick={closeMenu}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 840 840"
            className={styles.svgIcon}
          >
            <path d="M396.37 309.5H444.37V485.5C444.37 498.755 433.625 509.5 420.37 509.5C407.115 509.5 396.37 498.755 396.37 485.5V309.5Z" />
            <path d="M280.37 285.5C280.37 272.245 291.115 261.5 304.37 261.5H536.37C549.625 261.5 560.37 272.245 560.37 285.5C560.37 298.755 549.625 309.5 536.37 285.5Z" />
            <path d="M278.37 655.5H562.37L491.37 524.5H349.37L278.37 655.5Z" />
            <path d="M741.666 185C780.156 185 804.212 226.667 784.967 260L564.616 641.657L530.791 579.245L686.24 310C705.485 276.667 681.428 235 642.938 235H197.802C159.312 235 135.255 276.667 154.5 310L309.948 579.244L276.123 641.657L55.7733 260C36.5282 226.667 60.585 185 99.075 185H741.666Z" />
          </svg>
        </Link>

        <div className={styles.headerTools}>
          {/* 2. Search 區域 */}
          <HeaderSearchBar />

          {/* 3. 文字 wrappers */}
          <Link
            href="/highways"
            className={`${styles.navTextButton} text-wrapper`}
            onClick={closeMenu}
          >
            公路旅途
          </Link>
          <Link
            href="/railways"
            className={`${styles.navTextButton} text-wrapper`}
            onClick={closeMenu}
          >
            車站旅途
          </Link>

          {/* 4. Light mode switch */}
          <ThemeToggle />

          {/* 5. Home icon */}
          <Link
            href="/"
            className={`${styles.homeButton} icon-home`}
            aria-label="回首頁"
            onClick={closeMenu}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 48 48"
              className={styles.svgIcon}
            >
              <path d="M12 38H18V28C18 27.4333 18.192 26.9587 18.576 26.576C18.96 26.1933 19.4347 26.0013 20 26H28C28.5667 26 29.042 26.192 29.426 26.576C29.81 26.96 30.0013 27.4347 30 28V38H36V20L24 11L12 20V38ZM8 38V20C8 19.3667 8.142 18.7667 8.426 18.2C8.71 17.6333 9.10133 17.1667 9.6 16.8L21.6 7.8C22.3 7.26667 23.1 7 24 7C24.9 7 25.7 7.26667 26.4 7.8L38.4 16.8C38.9 17.1667 39.292 17.6333 39.576 18.2C39.86 18.7667 40.0013 19.3667 40 20V38C40 39.1 39.608 40.042 38.824 40.826C38.04 41.61 37.0987 42.0013 36 42H28C27.4333 42 26.9587 41.808 26.576 41.424C26.1933 41.04 26.0013 40.5653 26 40V30H22V40C22 40.5667 21.808 41.042 21.424 41.424C21.04 41.81 20.5653 42.0013 20 42H12C10.9 42 9.95867 41.6087 9.176 40.826C8.39333 40.0433 8.00133 39.1013 8 38Z" />
            </svg>
          </Link>

          <button
            type="button"
            className={`${styles.moreOptionButton} more-option`}
            aria-label="開啟選單"
            onClick={toggleMenu}
          >
            {isMenuOpen ? (
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                className={styles.svgStrokeIcon}
              >
                <line x1="18" y1="6" x2="6" y2="18" />
                <line x1="6" y1="6" x2="18" y2="18" />
              </svg>
            ) : (
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                className={styles.svgStrokeIcon}
              >
                <path d="M4.125 18.375H19.875M4.125 12.375H19.875M4.125 6.375H19.875" />
              </svg>
            )}
          </button>
        </div>
      </div>

      {/* 手機版下拉選單 */}
      <div
        className={`${styles.menuContainer} ${isMenuOpen ? styles.isMenuOpen : ""}`}
      >
        <div
          className={`${styles.menuContent} ${isMenuOpen ? styles.isMenuOpen : ""}`}
        >
          <Link
            href="/"
            className={`${styles.dropListHomeButton} icon-home`}
            aria-label="回首頁"
            onClick={closeMenu}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              className={styles.svgIcon}
            >
              <path d="M6 19H9V14C9 13.7167 9.096 13.4793 9.288 13.288C9.48 13.0967 9.71733 13.0007 10 13H14C14.2833 13 14.521 13.096 14.713 13.288C14.905 13.48 15.0007 13.7173 15 14V19H18V10L12 5.5L6 10V19ZM4 19V10C4 9.68333 4.071 9.38333 4.213 9.1C4.355 8.81667 4.55067 8.58333 4.8 8.4L10.8 3.9C11.15 3.63333 11.55 3.5 12 3.5C12.45 3.5 12.85 3.63333 13.2 3.9L19.2 8.4C19.45 8.58333 19.646 8.81667 19.788 9.1C19.93 9.38333 20.0007 9.68333 20 10V19C20 19.55 19.804 20.021 19.412 20.413C19.02 20.805 18.5493 21.0007 18 21H14C13.7167 21 13.4793 20.904 13.288 20.712C13.0967 20.52 13.0007 20.2827 13 20V15H11V20C11 20.2833 10.904 20.521 10.712 20.713C10.52 20.905 10.2827 21.0007 10 21H6C5.45 21 4.97933 20.8043 4.588 20.413C4.19667 20.0217 4.00067 19.5507 4 19Z" />
            </svg>
          </Link>
          <Link
            href="/highways"
            className={`${styles.dropListTextButton} text-wrapper`}
            onClick={closeMenu}
          >
            公路旅途
          </Link>
          <Link
            href="/railways"
            className={`${styles.dropListTextButton} text-wrapper`}
            onClick={closeMenu}
          >
            車站旅途
          </Link>
        </div>
      </div>
    </header>
  );
};

const MemoizedHeader = memo(Header);
MemoizedHeader.displayName = "Header";

export default MemoizedHeader;
