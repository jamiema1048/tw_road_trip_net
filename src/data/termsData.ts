export interface TermItem {
  subtitle: string;
  content: string;
}

export interface TermSection {
  title: string;
  items: TermItem[];
}

export const TERMS_DATA: TermSection[] = [
  {
    title: "一、 網站定位與非官方聲明（Nature of Service）",
    items: [
      {
        subtitle: "本站定位：",
        content:
          "本站為獨立個人開發之非商業文史與大眾交通資料庫，旨在進行歷史考證、地理軌跡重構與學術交流。",
      },
      {
        subtitle: "非官方代表：",
        content:
          "本站與中華民國交通部、臺灣鐵路公司、台灣糖業公司、林務局林鐵處等公私立營運機關無任何隸屬、代理或合作關係。網站內展示之徽章、標誌、名稱，其商標權仍屬原權利人所有，本站僅於合理範圍內作為辨識與教學使用。",
      },
    ],
  },
  {
    title: "二、 數據免責與第三方連結條款（Limitation of Liability）",
    items: [
      {
        subtitle: "不保證即時性與正確性：",
        content:
          "本站所提供之路線、車站、里程及幾何圖資，多屬歷史文獻整理，不保證其絕對正確性、完整性或即時性。",
      },
      {
        subtitle: "不承擔行程與人身損失：",
        content:
          "使用者不得將本站資料作為實際交通規劃、氣象防災、導航或登山探險之唯一依據。若因信賴本站資料而導致行程延誤、金錢損失、人身安全事故或任何衍生損害，本站不承擔任何法律與民事賠償責任。",
      },
      {
        subtitle: "第三方連結：",
        content:
          "本站可能包含指向官方網站（如台鐵車次查詢）或其他民間考證網頁的連結，該等外部網站之內容與隱私政策由其各自之營運者負責，本站概不負責。",
      },
    ],
  },
  {
    title: "三、 智慧財產權與合理使用規範（Intellectual Property）",
    items: [
      {
        subtitle: "個人與非商業使用：",
        content:
          "使用者可以自由瀏覽本站。在非商業目的（如學術論文引用、鐵道/公路迷社群討論、個人部落格）下，歡迎引用本站整理之文字與圖表，但必須註明出處與本站網址。",
      },
      {
        subtitle: "禁止商業利用與重製：",
        content:
          "未經明確授權，任何人不得將本站之程式碼、自製網頁元件、整合之資料庫內容用於任何商業營利行為、廣告載體或付費服務。",
      },
    ],
  },
  {
    title: "四、 網站使用限制與防爬蟲條款（Acceptable Use & Anti-Scraping）",
    items: [
      {
        subtitle: "禁止惡意攻擊：",
        content:
          "使用者不得嘗試繞過本站安全機制、注入惡意程式碼或進行阻斷服務攻擊（DoS）。",
      },
      {
        subtitle: "爬蟲與自動化限制：",
        content:
          "嚴禁使用任何自動化工具、網路爬蟲（Scrapers）、機器人程式（Bots）對本站資料庫進行高頻率、大批量的數據抓取。若有學術研究或非營利專案之大量數據需求，請主動聯繫本站取得 API 或 dump 授權，避免對伺服器造成負荷。",
      },
    ],
  },
  {
    title: "五、 條款修改與管轄法院（Modifications & Jurisdiction）",
    items: [
      {
        subtitle: "條款修改：",
        content:
          "本站保留隨時修改或變更本使用條款之權利，修改後之條款將直接公告於網頁，不另行個別通知。",
      },
      {
        subtitle: "準據法與管轄：",
        content:
          "本服務條款之解釋與適用悉依中華民國法律。若因本網站服務產生任何爭議，雙方同意以台灣高雄地方法院為第一審管轄法院。",
      },
    ],
  },
];
